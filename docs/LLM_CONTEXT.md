...
- Current-project focus mode: when launched inside a linked project the TUI lands on that project's profiles screen and hides the multi-project list until `p` unlocks browsing (`m.browseProjects`); the one-shot landing is gated by `m.landed` in the `reloadMsg` handler so migration/unlock defer it → `internal/tui/tui.go`, `internal/tui/keys.go` (`isFocusedProject`), `internal/tui/focus_test.go`.
- Diff viewer scope: `syncDiffScope()` returns the current project when `isFocusedProject()`, else "" (all). It filters the value-free inventory at render and is passed through `revealSyncDiffCmd` → `app.RevealSyncLineDiff(scope)` → `vault.CompareVaultSnapshotLines(scope)` so a focused reveal only decrypts the scoped project. `syncDiffReturn`/`syncDiffReturnScreen()` send esc and post-action back to the origin screen (profiles vs project list) → `internal/tui/sync_diff_actions.go`, `internal/tui/sync_diff_view.go`, `internal/tui/sync_inventory_view.go`, `internal/app/sync_line_diff.go`, `internal/vault/diff.go`.
...
- Distribution: `scripts/build-release.sh` cross-compiles static (`CGO_ENABLED=0`, `-trimpath`, `-ldflags "-s -w -X main.version=<tag>"`) binaries named `gitenv_<os>_<arch>[.exe]` for linux/darwin/windows × amd64/arm64 into `dist/` plus `checksums.txt`. `.github/workflows/release.yml` builds and publishes them as GitHub release assets on `v*` tags. `install.sh`/`install.ps1` detect os/arch, fetch the matching asset from `releases/latest/download` (or `GITENV_VERSION`), verify the SHA-256 checksum, and install to `~/.local/bin` / `%LOCALAPPDATA%\gitenv\bin`. `main.version` is the single injected version string.
- `gitops.CommitAndPush` commits vault changes with an explicit identity (`-c user.name=gitenv -c user.email=gitenv@localhost`) so publishing never depends on the user's global git config (and CI, which has none, works). Tests that drive their own git repos still set identity via their `runGit`/`commitVault` helpers.
